...
AddEventHandler('JewelRob:GiveItems', function(amount)
    ...
    if suckme == 1 then 
        ...
        xPlayer.addInventoryItem(g, (amount - 2))
    end
...
RegisterServerEvent('LilBitchWhereTheGold')
AddEventHandler('LilBitchWhereTheGold', function()
	local _source = source
	local xPlayer  = ESX.GetPlayerFromId(source)
	if xPlayer.getInventoryItem("GoldBar").count >= 75 then
		xPlayer.removeInventoryItem("GoldBar", 75)
		xPlayer.addMoney(70000)

	else
		---send shit to them saying not enoguh
	end
end)