...
Citizen.CreateThread(function()
    while true do
		...
		local SellGold = vector3(-99.47, 6461.34, 31.63)
		local playerCoords = GetEntityCoords(PlayerPedId())
        local dist = #(vector3(SellGold['x'],SellGold['y'],SellGold['z']) - playerCoords)
        if dist < 30 then
            sleep = 50
            if dist < 3 then
                sleep = 6
                DrawText3Ds(SellGold['x'],SellGold['y'],SellGold['z'],'[E] To Sell 75 Gold Bars For $70,000')
                if IsControlJustPressed(2, 86) then
                    TriggerServerEvent('LilBitchWhereTheGold')
                end
            end
        end
        Citizen.Wait(sleep)
